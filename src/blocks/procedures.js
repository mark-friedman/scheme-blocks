...
const { FieldParameterFlydown } = LexicalVariablesPlugin;
import { chameleonMixin } from './mixins.js';
import { blocklyMixin } from '../utils/mixin.js';
...
const procedureCallBase = (isGeneric, blockName, argNames = []) => {
  return {
    init: function () {
      ...
      this.setStyle('procedure_blocks');
      if (isGeneric) {
        this.setMutator(new Blockly.icons.MutatorIcon(['procedures_call_item'], this));
      }
    },
    ...
    compose: function (topBlock) {
      ...
      for (let i = 0; i < this.itemCount; i++) {
        connections[i]?.connect(this.getInput('ARG' + i)?.connection);
      }
    },
    /**
